...
// CONEXÃO COM O BANCO DE DADOS POSTGRES (RENDER)
const pool = new Pool({
...
app.get("/api/dashboard/:store", async (req, res) => {
  ...
  try {
    ...
    // Retorna drivers online filtrados pela loja
...
app.post("/register-delivery", async (req, res) => {
  ...
  try {
    await ensureTables();
    await pool.query(
      "INSERT INTO orders_v3 (id, store_slug, client_name, address, phone, price, lat, lng, delivery_code, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'pending')",
...
io.on("connection", (socket) => {
  ...
  socket.on("disconnect", () => {
      // Mantemos o motoboy na lista por 10 min mesmo se o socket cair
  });
});

// Limpeza de motoboys inativos (timeout de 10 minutos)
setInterval(() => {
    const timeout = Date.now() - (10 * 60 * 1000);
    onlineDrivers = onlineDrivers.filter(d => d.lastSeen > timeout);
}, 60000);

server.listen(process.env.PORT || 3000, () => console.log("Servidor Logística iGO V3.5 Online"));
